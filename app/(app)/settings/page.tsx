...
import {
  ...
  ChevronRight, Trash2, X, Check, Pencil,
} from "lucide-react";
...
const TABS = ["Profile", "Security", "Users", "Config"] as const;
  ...
  const deactivate = useMutation({
    mutationFn: (id: number) =>
      axios.delete(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000/api"}/users/${id}`, {
        headers: { Authorization: `Bearer ${localStorage.getItem("gpa_token")}` },
      }),
    onSuccess: () => {
...
export default function SettingsPage() {
  ...
  const visibleTabs = TABS.filter((t) =>
    t !== "Users" && t !== "Config" ? true : isSuperAdmin
  );
